...
static void usage(const char *progname)
{
    fprintf(stderr,
            ...
            "-e <hash|pathname>\tLoad specified Moxie executable into address "
            "space\n"
...
static void sandboxInit(machine &mach,
                        int argc,
                        char **argv,
                        string &outFilename,
                        string &gmonFilename,
...
static uint32_t readHexValueFixedLength(char *buffer, int *index, int length)
{
    int n = 0;
    int i = *index;
            ...
            case '$': {
                ...
                case 'g': {
                    ...
                    for (ri = 0; ri < 17; ri++) {
                        ...
                        sprintf(&reply[ri * 8], "%02x%02x%02x%02x",
                                (rv >> 0) & 0xff, (rv >> 8) & 0xff,
                                (rv >> 16) & 0xff, (rv >> 24) & 0xff);
                    }
                    sendGdbReply(newsockfd, reply);
                    i += 4;
                } break;
